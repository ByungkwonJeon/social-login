 ...
 * "https://openid.net/specs/openid-connect-core-1_0.html#IssuerIdentifier">Issuer</a> in a signed
 * JWT (JWS).
 * <p>
 * To use, this class must be able to determine whether or not the `iss` claim is trusted. Recall
 * that anyone can stand up an authorization server and issue valid tokens to a resource server. The
 * simplest way to achieve this is to supply a list of trusted issuers in the constructor.
 * <p>
 ...
 * @since 5.3
 */
public final class MyJwtIssuerAuthenticationManagerResolver implements
    AuthenticationManagerResolver<HttpServletRequest> {

  private final AuthenticationManager authenticationManager;

  /**
   * Construct a {@link MyJwtIssuerAuthenticationManagerResolver} using the provided parameters
   *
   * @param trustedIssuers a list of trusted issuers
   */
  public MyJwtIssuerAuthenticationManagerResolver(String... trustedIssuers) {
   ...
   * @param trustedIssuers a list of trusted issuers
   */
  public MyJwtIssuerAuthenticationManagerResolver(Collection<String> trustedIssuers) {
   ...
   */
  public MyJwtIssuerAuthenticationManagerResolver(
      AuthenticationManagerResolver<String> issuerAuthenticationManagerResolver) {
    Assert.notNull(issuerAuthenticationManagerResolver,
        "issuerAuthenticationManagerResolver cannot be null");
    this.authenticationManager = new ResolvingAuthenticationManager(
        issuerAuthenticationManagerResolver);
  }

  /**
   * Return an {@link AuthenticationManager} based off of the `iss` claim found in the request's
   * bearer token
   *
   * @throws OAuth2AuthenticationException if the bearer token is malformed or an
   *                                       {@link AuthenticationManager} can't be derived from the
   *                                       issuer
   */
  @Override
  ...
  private static class ResolvingAuthenticationManager implements AuthenticationManager {
    ...
    public Authentication authenticate(Authentication authentication)
      ...
      AuthenticationManager authenticationManager = this.issuerAuthenticationManagerResolver.resolve(
          issuer);
  ...
  private static class JwtClaimIssuerConverter implements
      Converter<BearerTokenAuthenticationToken, String> {
  ...
  static class TrustedIssuerJwtAuthenticationManagerResolver implements
    ...
    public AuthenticationManager resolve(String issuer) {
      if (this.trustedIssuer.test(issuer)) {
        AuthenticationManager authenticationManager = this.authenticationManagers.computeIfAbsent(
            issuer,
            (k) -> {
              this.logger.debug("Constructing AuthenticationManager");
              JwtDecoder jwtDecoder = JwtDecoders.fromIssuerLocation(issuer);
              return new MyJwtAuthenticationProvider(jwtDecoder)::authenticate;
            });
        this.logger.debug(
            LogMessage.format("Resolved AuthenticationManager for issuer '%s'", issuer));
